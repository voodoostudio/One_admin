<ul class="m-menu__nav  m-menu__nav--dropdown-submenu-arrow ">
    ...
    @foreach ($items as $item)
        ...
        <li id="{{ mb_strtolower($item->title) }}" class="m-menu__item  m-menu__item--{{ (implode(" ", $listItemClass) == 'active') ? 'active' : 'submenu'  }}" aria-haspopup="true" {{ (implode(" ", $listItemClass) == 'active') ? '' : 'data-menu-submenu-toggle=hover'  }} {{($item->title == 'Tools' && Auth::user()->role_id != 1 || $item->title == 'Clients' && Auth::user()->role_id == 5) ? 'style=display:none' : ''}} >
